...
@Component({
  ...
  template: `
    <div
      class="row mt mb d-flex justify-content-center justify-content-md-between"
    >
      ...
      <app-post
        *ngFor="let post of posts | postRelacionados"
        [post]="post"
        class="col-12 col-md-4 d-flex justify-content-center mb-5 mb-md-0"
      ></app-post>
    </div>
  `,
  styles: [
...
export class PostRelacionadosComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.butter.getPost(this.slug).then((res) => {
      this.categorie = res.data.data.categories?.[0].slug;

      this.butter.getRelacionados(`${this.categorie}`).then((post) => {
